...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
           best_pos.append(max(i, key= i.get))

        cloestest_ghost = {}
        for i in best_pos:
            cloestest_ghost[i] = self.distancer.getDistance(pacmanPosition, i)

        close = min(cloestest_ghost, key= cloestest_ghost.get)

        minimum = {}
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            minimum[action] = self.distancer.getDistance(successorPosition, close)

        best = min(minimum, key= minimum.get)
        return best
